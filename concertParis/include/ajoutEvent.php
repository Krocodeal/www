...
if (!isset($_SESSION['user']) || $_SESSION['user']['PERSONNE_TypeUser'] != 'organisateur') {
...
if (isset($_POST['evenement']) && ($_POST['evenement'] == 'Ajouter')) {
    ...
    {	
		...
		//---------------------------------------------------------------------------------------------------------------------
		//Insert des styles de musique de l'event dans la table animer

		$aSelectedValuesS = $_POST['style'];
		
		foreach($aSelectedValuesS as $selectValue){
		
			$sql = 'INSERT INTO animer VALUES("' . $selectValue . '", 
						"' . $lastId . '
						")';
			mysql_query($sql) or die('Erreur SQL !' . $sql . '<br />' . mysql_error());
		...
		//---------------------------------------------------------------------------------------------------------------------
		//Insert des artistes dans la table participer

		$aSelectedValuesA = $_POST['artiste'];
		
		foreach($aSelectedValuesA as $selectValue){
		
			$sql = 'INSERT INTO participer VALUES("' . $selectValue . '", 
						"' . $lastId . '
						")';
			mysql_query($sql) or die('Erreur SQL !' . $sql . '<br />' . mysql_error());
...
?>
	...
	<div class="post">
		<form action="#" method="post" enctype="multipart/form-data">
			<fieldset>
			<legend>Evenement :</legend><br />
				
				<label for="libelle">Libellé de l'évenement : </label>
				<input type="text" name="libelle" value="<?php if (isset($_POST['libelle'])) echo htmlentities(trim($_POST['libelle'])); ?>">*<br />				
				
				<label for="photo">Photo de l'évenement : </label>
				...
				</select>*<a href="ajoutLieu.php" ><img src="http://www.iconshock.com/img_jpg/IMPRESSIONS/general/jpg/16/add_icon.jpg"></a><br />
				...
				</select>*<a href="inscription.php" ><img src="http://www.iconshock.com/img_jpg/IMPRESSIONS/general/jpg/16/add_icon.jpg"></a><br />

				<label for="orga" >Organisateur : </label>
				<input type="text" name="orgaV" value="<?php if (isset($_SESSION['user']['PERSONNE_Nom'])) echo htmlentities(trim($_SESSION['user']['PERSONNE_Nom'])); ?>">*<br />
				<input type="hidden" name="orga" value="<?php if (isset($_SESSION['user']['PERSONNE_ID'])) echo htmlentities(trim($_SESSION['user']['PERSONNE_ID'])); ?>"><br />
				...
				</select>*<a href="#.php" ><img src="http://www.iconshock.com/img_jpg/IMPRESSIONS/general/jpg/16/add_icon.jpg"></a><br />
